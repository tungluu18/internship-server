...
module.exports = {
    ...
    getMarkAndCommentForStudent: async function(lectureId){
        try{ 
            ...
            for (var i=0; i<idd.length ;i++){
                ...
                res = await knex.from('studentfollowme').where('studentId',idd[i]).innerJoin('student','studentfollowme.studentId','student.id').select('id','name','commentOfLecture','Mark')
    ...
    downloadCommentAndMarkForStudent: async function(lectureId){
        ...
        try{
            ...
            var dir = 'C:/AppServ/www/internship-server/data/danhgiasinhvien/'+ lectureId+'.xlsx';
